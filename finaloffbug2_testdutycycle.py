        ...
        if cal_temperature < min_temp or cal_temperature > max_temp:
            ...
            if shower_status == "on":  # Check if the shower is on before adjusting the flow
                if cal_temperature - desired_temp > accept_temp_diff:
                    duty_cycle = min(cal_temperature - desired_temp, hot_duty_cycle - 1, 57 - cold_duty_cycle)
                    # Reduce hot water flow,
                    if hot_duty_cycle > 1:
                        hot_duty_cycle -= duty_cycle
                    if cold_duty_cycle < 57:
                        cold_duty_cycle += duty_cycle
                elif desired_temp - cal_temperature > accept_temp_diff:
                    duty_cycle = min(desired_temp - cal_temperature, 57 - hot_duty_cycle, cold_duty_cycle - 1)
                    ...
                    # Temperature is within the desired range, no need to adjust the flow
                    # Keep the servos at the current position (desired temperature)
                    print("Currently in acceptable range")
