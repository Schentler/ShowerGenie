...
def temperature_controller(data):
    global previous_temperature, guest_name, shower_status, servo_pos, desired_temp
    hot_duty_cycle = 57 - servo_pos
    cold_duty_cycle = servo_pos
    desired_temp_received = False

    while not desired_temp_received:
        cal_temperature = read_temp()
        print(f"Current temp: {cal_temperature}°C, Waiting for desired temperature...")

        if shower_status == "on" and "desiredTemperature" in data:
            desired_temp_str = data.get("desiredTemperature")
            set_desired_temperature(desired_temp_str)
            desired_temp_received = True
            print(f"Desired temperature received: {desired_temp}°C")
    ...
    while True:
        ...
        if cal_temperature < min_temp or cal_temperature > max_temp:
            ...
            if shower_status == "on":  # Check if the shower is on before adjusting the flow
                ...
                if cal_temperature != previous_temperature and command == "On":
...
try:
    ...
    temp_controller_thread = threading.Thread(target=temperature_controller, args=(received_data,))
