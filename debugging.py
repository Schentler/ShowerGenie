...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        if command == "On":
            # Wait for the desired temperature to be available in the payload
            while "desiredTemperature" not in data:
                print("Waiting for desired temperature...")
                time.sleep(1)
                payload = msg.payload.decode("utf-8")
                data = json.loads(payload)
...
def temperature_controller():
    ...
    while True:
        # Fetch the latest payload to get any updates
        client.loop()
